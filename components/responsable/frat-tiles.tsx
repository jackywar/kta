    ...
    <ul className="grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
      {frats.map((f) => (
        <li key={f.id}>
          ...
          >
            <article
              className="relative flex aspect-square flex-col overflow-hidden rounded-2xl border border-border shadow-sm transition-shadow hover:shadow-md"
              style={{ backgroundColor: f.color_oklch }}
            >
              <div className="absolute inset-0 bg-black/25" aria-hidden />
              <div className="relative flex h-full flex-col p-4 text-primary-foreground">
                ...
                <div className="mt-2 min-h-0 flex-1">
                  ...
                  <ul className="mt-1 space-y-0.5 text-xs text-primary-foreground/90">
                    {(f.responsables ?? [])
                      .map((x) => x.profile)
                      .filter(Boolean)
                      .slice(0, 4)
                    ...
                    {(f.responsables ?? []).filter((x) => x.profile).length > 4 ? (
                      <li className="text-primary-foreground/80">…</li>
                    ) : null}
                  </ul>
                </div>

                <div className="mt-3 rounded-xl bg-card/15 px-3 py-2 text-xs font-medium">
